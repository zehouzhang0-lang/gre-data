...
生词本的“分类与关联”入口提供主题浏览、近义词、反义词和形近词检索。初始数据在 `vocab/relations.json`：17个可改名、移除和恢复的主题，35组逐词用法对照。主题根据现有中文释义和明确词表提出建议，支持多主题；没有把握的词保留待分类。单词分类可以手动覆盖或恢复自动建议。当前初始规则覆盖381/808词，其余包含211个尚无释义的词；不把覆盖率当成学习掌握度。

关系组支持新增、编辑、移除及恢复，成员从已有生词本选择；移除词组不删除单词。近义关系仅限说明中的义项，不能自动判定SE等价。初始辨析分为已查词典与待核草稿，编辑后标记自己整理。词组详情提供逐词词典入口。“对照记忆”先隐藏用法，揭晓不写学习证据；“背诵这一组”及“背诵当前词表”接入原有回忆自评。
...
“记忆与背诵”现为每日复习入口，汇总到期词数、到期题数、今日已完成与实际复习次数，并展示未来7天当前已排定的量。任务支持按类别、到期状态和词义检索，逐项查看累计次数、下次日期和最近20次历史；完整事件仍保留。单词每批20个、题目每批10道，逾期积压保留，可以分批完成。待首轮单独列出；没有释义且无可用反馈的词可在任务行补充释义。
