...
export default function ReviewDashboard({state,onWords,onQuestions,refresh}) {
  ...
  return <>
    ...
    <div className="review-overview">
      ...
      <section className="review-done"><span>今日已完成复习</span><strong>{completed}<small>项</small></strong><p>{reviewed} 项已练 · 共 {repetitions} 次复习</p><small>忘记后需再练的内容仍算待完成。另有 {summary.word.first_today} 词、{summary.question.first_today} 题完成首轮。</small><button disabled={!newWords.length} onClick={()=>startWords(newWords.slice(0,10))}>首轮回忆 · {newWords.length} 词可练</button></section>
    </div>
    ...
    <section className="review-inventory"><div className="table-tools"><div className="tabs"><button className={kind==='word'?'selected':''} onClick={()=>{setKind('word');setPage(0);}}>单词</button><button className={kind==='question'?'selected':''} onClick={()=>{setKind('question');setScope(s=>s==='new'?'all':s);setPage(0);}}>做过的题</button></div><input aria-label="搜索复习记录" placeholder="搜索单词、释义或题目…" value={search} onChange={e=>{setSearch(e.target.value);setPage(0);}}/><select aria-label="复习任务筛选" value={scope} onChange={e=>{setScope(e.target.value);setPage(0);}}><option value="due">今日待复习</option><option value="all">全部记录</option>{kind==='word'&&<option value="new">待首轮</option>}</select></div>
      {visible.slice(currentPage*20,(currentPage+1)*20).map(i=><article className="review-item" key={i.id}><div><button className="word-link" onClick={()=>setDetail(i)}>{i.word||questionTitle(i)}</button><small>{i.kind==='question'?`已做 ${i.attempt_count} 次 · `:''}已复习 {i.review_count} 次 · {nextLabel(i)}{!i.ready?' · 待补释义':''}</small></div><button onClick={()=>!i.ready?(setEditing(i),setMeaning(''),setError('')):i.kind==='word'?startWords([i]):onQuestions([i])}>{i.ready?'再练一次':'补充释义'}</button></article>)}
      {!visible.length&&<Empty title="当前筛选下没有任务"><p>可以查看全部记录，或开始首轮回忆。</p></Empty>}
      {visible.length>20&&<nav className="table-tools" aria-label="复习任务分页"><button disabled={currentPage===0} onClick={()=>setPage(currentPage-1)}>上一页</button><span>{currentPage+1} / {pages} · {visible.length} 项</span><button disabled={currentPage+1===pages} onClick={()=>setPage(currentPage+1)}>下一页</button></nav>}
    </section>
    <details className="review-method"><summary>复习如何安排</summary><p>一轮单词学习依次练认义、补全字母和中文回忆英文。认义默认两遍，错误穿插重练；每次小练习保留记录，同一个词完成本轮只结算一次复习。纠正后答对不会抹去本轮最初的困难。</p><p>跨天继续采用间隔复习：顺利回忆后按 {data.intervals.join('、')} 天逐步延长；忘记或答错，10 分钟后再练；不够准确或缺少答案，次日再练。提前或同日重复成功不连续跨级，未完成任务保留。</p><p>单词小练习核对所给义项与拼写，旧翻卡记录仍是自评；题目按提交时已有的参考答案安排。练习通过不自动认定长期掌握。这里的轮次与间隔是可调整的初始设置，并非人人通用的最佳遗忘曲线。<a href="https://www.psychologicalscience.org/publications/journals/pspi/learning-techniques.html" target="_blank" rel="noreferrer">主动提取与间隔学习依据 ↗</a></p></details>
