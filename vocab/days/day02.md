---
schema_version: 1
day: 2
date: "2026-09-06"
plan_date: "2026-09-03"
status: in_progress
source:
  material_id: core3000_random_2026
  units: [list2]
  item_range: "1-20"
target_count: 120
reviewed_count: 20
recall_test:
  total: 20
  correct: 15
  partial: 4
  wrong: [vicarious]
partial_words: [antiquity, symbiosis, vilify, embody]
difficult_added: [antiquity, symbiosis, vicarious, vilify, embody]
mastered_today: []
duration_min: null
verification_sources:
  - https://www.merriam-webster.com/dictionary/antiquity
  - https://www.merriam-webster.com/dictionary/symbiosis
  - https://www.merriam-webster.com/dictionary/assent
  - https://www.merriam-webster.com/dictionary/vicarious
  - https://www.merriam-webster.com/dictionary/potent
  - https://www.merriam-webster.com/dictionary/vilify
  - https://www.merriam-webster.com/dictionary/accede
  - https://www.merriam-webster.com/dictionary/embody
---

# Day 2 词汇记录（进行中）

目前完成 list2 第 1–20 词的主动回忆：15 词完全正确、4 词部分正确、1 词错误。记录日期采用真实作答日期 2026-09-06；`plan_date` 保留原 Day 2 日期，不回填 9 月 3–5 日的虚假完成记录。

## 部分正确

- `antiquity`：回答“古董”抓到了古代事物的方向，但单数核心义优先是“古代；古老”。“古代遗物”通常用复数 `antiquities`；普通“古董”更常用 `antique`。
- `symbiosis`：概念方向正确，但它是名词“共生（关系）”；形容词“共生的”是 `symbiotic`。
- `vilify`：回答“诬陷”接近损害名誉，但核心是“公开诋毁、中伤”；不要求一定捏造具体罪名。
- `embody`：回答“象征”方向接近，但核心是“体现、使具体化；包含”。`symbolize` 才是一般意义的“象征”。

## 错误

- `vicarious`：未回忆出。GRE 常见核心义为“通过想象或同感间接体验他人经历的”；另有“替代的、代位的”。

## 正确但需固定搭配

- `assent`：核心义“同意、赞成”正确；常见搭配为 `assent to a proposal`，不要把它泛化成所有语境下的“允许”。
- `potent`：回答“有权势的”正确；还需保留常见核心义“强有力的、有效力的”。
- `accede`：回答“认可、加入”正确；常见结构是 `accede to a demand/agreement`，另可表示“就任、继位”。

## 下一次检验

对 `antiquity`、`symbiosis`、`vicarious`、`vilify`、`embody` 做无提示复测；连续 3 次主动回忆正确后才进入 mastered。
